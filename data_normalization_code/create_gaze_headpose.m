...
% save as above, conver head pose to the polar coordinate system     
                ...
                save_path_gaze_l = strcat('F:\datebase\zhang_xucong_paper\MPIIGaze\MPIIGaze\Data\gaze_2D\P',int2str(k),'\day',int2str(i),'\left_gaze.txt')
                fp_g_l = fopen (save_path_gaze_l,'a+')
                fprintf(fp_g_l,' %s',str2mat(image_name))
                fprintf(fp_g_l,' %s',num2str(gaze_theta_L))
                fprintf(fp_g_l,' %s',num2str(gaze_phi_L))
                fprintf(fp_g_l,'\n')
                fclose(fp_g_l);
                save_path_gaze_r = strcat('F:\datebase\zhang_xucong_paper\MPIIGaze\MPIIGaze\Data\gaze_2D\P',int2str(k),'\day',int2str(i),'\right_gaze.txt')
                fp_g_r = fopen (save_path_gaze_r,'a+')
                fprintf(fp_g_r,' %s',str2mat(image_name))
                fprintf(fp_g_r,' %s',num2str(gaze_theta_R))
                fprintf(fp_g_r,' %s',num2str(gaze_phi_R))
                fprintf(fp_g_r,'\n')
                fclose(fp_g_r);
                save_headpose_l =strcat('F:\datebase\zhang_xucong_paper\MPIIGaze\MPIIGaze\Data\headpose_2D\P',int2str(k),'\day',int2str(i),'\left_headpose.txt')
                fp_h_l = fopen (save_headpose_l,'a+')
                fprintf(fp_h_l,' %s',str2mat(image_name))
                fprintf(fp_h_l,' %s',num2str(headpose_theta_L))
                fprintf(fp_h_l,' %s',num2str(headpose_phi_L))
                fprintf(fp_h_l,'\n')
                fclose(fp_h_l);
                save_headpose_r =strcat('F:\datebase\zhang_xucong_paper\MPIIGaze\MPIIGaze\Data\headpose_2D\P',int2str(k),'\day',int2str(i),'\right_headpose.txt')
                fp_h_r = fopen (save_headpose_r,'a+')
